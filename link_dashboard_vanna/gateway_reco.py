...
class MyPrivateVanna:
    ...
    - version (integer) -- Record version tracker"""
        ...
        prompt = (
            f"You are a strict, zero-hallucination PostgreSQL translation engine.\n"
            f"Your task is to convert this natural language request into a query matching the schema exactly: {question}\n\n"
            
            f"=== AVAILABLE DATABASE METADATA STRUCTURAL SCHEMAS ===\n"
            f"{schema_context}\n\n"
            ...
            f"1. CHARACTER-PERFECT ALIGNMENT:\n"
            f"   - You must copy table names and column identifiers EXACTLY as spelled in the context above.\n"
            f"   - Do not change singular words to plural, do not drop characters, and do not truncate word endings.\n"
            f"   - EVERY table name reference must maintain its original schema prefix (e.g., 'akademik.table_name').\n\n"
            
            f"2. COMPILATION AND SYNTAX SEGREGATION RULES:\n"
            f"   - Always declare explicit short table aliases in your FROM/JOIN definitions (e.g., 'FROM akademik.example_table AS e').\n"
            f"   - Explicitly prefix every single evaluated column name inside SELECT, WHERE, GROUP BY, and ORDER BY blocks with its assigned alias indicator variable (e.g., 'e.column_name').\n"
            f"   - GROUP BY matches: When grouping by an aggregated field column, ensure the GROUP BY columns exactly match the non-aggregated SELECT fields, or use standard column position indices (e.g., GROUP BY 1).\n\n"

            f"3. FUZZY TEXT MATCHING (CRITICAL): When filtering by text or string columns (e.g., jobs, cities, names), ALWAYS use PostgreSQL's case-insensitive `ILIKE '%keyword%'` operator instead of exact `=` matches. Data often contains compound values (e.g., 'Petani/ nelayan' or 'S1 Teknik'), so using `ILIKE '%Petani%'` ensures you do not miss records.\n"
            
            f"4. UNANSWERABLE SAFE FAILURE ENCODING:\n"
            f"   - If the request targets fields, entities, or entities that are absent from the context schema list, do not fake a query.\n"
            f"   - If unanswerable, bypass JSON output entirely and return exactly this plain text string (no brackets, no markdown code blocks):\n"
            ...
            f"5. OUTPUT PROTOCOL (STRICT PLAIN JSON):\n"
            f"   - If answerable, respond ONLY with a single valid, parsable JSON block matching the key template properties below.\n"
            f"   - CRITICAL: Do NOT embed markdown code wrappers (like ```sql or ```json) inside the JSON value fields or around the text body string block.\n"
            f"   - Ensure the entire text string value for the 'sql' parameter is placed on a single continuous text string line to avoid escaping problems.\n\n"
            
            f"=== TARGET JSON CONFIGURATION BLANK BLUEPRINT ===\n"
            f"{{\n"
            f"  \"sql\": \"SELECT a.col1, SUM(a.col2) AS total FROM akademik.table AS a GROUP BY a.col1 ORDER BY a.col1 ASC\",\n"
            f"  \"type\": \"line\"\n"
            f"}}\n"
            f"Use 'line' for temporal time trends, 'bar' for categories, and 'pie' for structural metric ratios.\n"
...
def ask_ai():
    try:
        ...
        raw_llm_output = vn.generate_sql(user_question).strip()
        print(f"[SERVER API]: Raw LLM Pipeline Output ->\n{raw_llm_output}")

        # Default fallback initial structural assignments
        sql_query = ""
        chart_type = "line"

        try:
            # 1. Clean out code blocks AND any stray "json" labels before the opening bracket
            ...
            if start_idx != -1 and end_idx != 0:
                ...
                try:
                    llm_payload = json.loads(clean_json_str)
                    sql_query = llm_payload.get("sql", "").strip()
                    chart_type = llm_payload.get("type", "line").strip()
                    
                    # Malformed inner markdown syntax cleanup
                    sql_query = re.sub(r'^```sql\s*|^```\s*|```$', '', sql_query, flags=re.IGNORECASE).strip()
                    if sql_query.lower().startswith("sselect"):
                        sql_query = sql_query[1:].strip()
                except Exception as parse_error:
                    print(f"[SERVER API]: JSON string parsing failed. Error: {str(parse_error)}")
                    sql_query = raw_llm_output
            else:
                # This now correctly pairs with: if start_idx != -1
                print("[SERVER API]: No JSON bracket markers discovered in string.")
                sql_query = raw_llm_output
            
        except Exception as parse_error:
            print(f"[SERVER API]: JSON parsing fell back. Error: {str(parse_error)}")
            sql_query = raw_llm_output

        print(f"[SERVER API]: Extracted SQL Statement -> {sql_query}")
        print(f"[SERVER API]: Extracted Chart Presentation Engine -> {chart_type}")

        # Now your safety check evaluates the actual SQL string, not the raw JSON block!
        is_rejection = (
        ...
        if is_rejection:
            ai_reply = "I apologize, but the academic data warehouse currently does not contain this data details."
            records = []
            query_id = ""
            ...
            ai_reply = f"**Time to answer: {elapsed_time2}**\n\n**Generated SQL Query:**\n```sql\n{sql_query}\n```\n\n"
            if len(records) == 0:
        ...
        return jsonify({
            "message": {
                ...
                "query_id": query_id,
                "raw_data": records,
                "chart_type": chart_type  # <-- Now dynamically outputs the value provided by Qwen!
            },
...
if __name__ == '__main__':
    # Running on local loopback interface (Port 5052) because 5050 and 5051 already taken to route traffic from HTML frontend
